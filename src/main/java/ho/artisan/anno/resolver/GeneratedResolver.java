...
public class GeneratedResolver extends ModelResolver<Generated> {
    ...
    public void process(Entry entry, Registration registration) {
        if (entry.is(Item.class)) {
            ...
            if (suffix.isBlank())
                items.add(generator -> generator.register(item, Models.GENERATED));
            else
                items.add(generator -> generator.register(item, suffix, Models.GENERATED));
        }
    }

    @Override
    public Class<Generated> aClass() {
