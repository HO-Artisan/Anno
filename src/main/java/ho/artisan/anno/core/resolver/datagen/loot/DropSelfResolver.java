...
public class DropSelfResolver implements DataGenResolver<DropSelf> {
    ...
    public void apply(FabricDataGenerator generator, FabricDataGenerator.Pack pack, Class<?> registration) {
        pack.addProvider((FabricDataGenerator.Pack.RegistryDependentFactory<DataProvider>) (output, registriesFuture) -> new DropSelfLootTableProvider(output, registriesFuture, blocks, name()));
    }
